...
struct ChatRequestBuilder {
    ...
    - 默认简短、针对性强：优先给结论与关键步骤，避免冗长铺垫。
    - 在用户未要求“详细展开”时，控制在短段落内完成回答；只保留必要信息。
    ...
    9) 若某语言在当前环境常见坑较多，先在 README 写清环境要求与版本。
    10) 输出时尽量给出简短步骤日志（每行一个动作，如“写入 src/main.cpp”“构建测试”），方便前端渲染进度胶囊。
    11) 除非用户明确要求解释，尽量以“简短说明 + 完整文件输出”为主。
